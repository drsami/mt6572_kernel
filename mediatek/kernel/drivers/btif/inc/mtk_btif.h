...
#define BTIF_RX_BUFFER_SIZE (1024 * 16)
...
#define BTIF_RX_BTM_CTX BTIF_THREAD_CTX//BTIF_TASKLET_CTX  
/*-- cannot be used because , mtk_wcn_stp_parser data will call *(stp_if_tx) to send ack, in which context sleepable lock or usleep operation may be used, these operation is not allowed in tasklet, may cause schedule_bug*/
...
typedef struct _btif_buf_str_
{
    ...
    unsigned int rd_idx;  /*For Tx: next Tx data pointer to FIFO; For Rx: next read data pointer from BTIF user*/
    unsigned int wr_idx; /*For Tx: next Tx data pointer from BTIF user; For Rx: next write data(from FIFO) pointer*/
}btif_buf_str, *p_btif_buf_str;


/*---------------------------------------------------------------------------*/
typedef struct _mtk_btif_dma_ {
    /*p_mtk_btif*/void *p_btif;          /*BTIF pointer to which DMA belongs*/
...
typedef struct _mtk_btif_{
    ...
    struct mutex  rx_mtx;     /*lock to BTIF's rx bottom half when kernel thread is used*/
    ...
    spinlock_t    rx_tasklet_spinlock;        /*lock to BTIF's rx bottom half when tasklet is used*/
...
typedef struct _mtk_btif_user_{
    ...
    char u_name[BTIF_USER_NAME_MAX_LEN];    /*BTIF's user, static allocation*/
    unsigned int u_id;
    p_mtk_btif p_btif;
...
#define BBS_COUNT(ptr) ((ptr)->wr_idx >= (ptr)->rd_idx ? (ptr)->wr_idx - (ptr)->rd_idx : BBS_SIZE(ptr) - ((ptr)->rd_idx - (ptr)->wr_idx))
#define BBS_COUNT_CUR(ptr, wr_idx) (wr_idx >= (ptr)->rd_idx ? wr_idx - (ptr)->rd_idx : BBS_SIZE(ptr) - ((ptr)->rd_idx - wr_idx))
...
#define BBS_WRITE_MOVE_NEXT(ptr) ((ptr)->wr_idx = ((ptr)->wr_idx + 1) & BBS_MASK(ptr))
#define BBS_READ_MOVE_NEXT(ptr) ((ptr)->rd_idx = ((ptr)->rd_idx + 1) & BBS_MASK(ptr))
