...
p_mtk_btif btif_exp_srh_id(unsigned int u_id)
{
    ...
    for (index = 0; (index < BTIF_PORT_NR) && (NULL == p_btif); index++)
    {
        ...
        list_for_each(tmp, p_list)
        {
            ...
            if (u_id == p_user->u_id)
            {
                ...
                BTIF_DBG_FUNC("BTIF's user id(0x%08x), p_btif(0x%08x)\n", p_user->u_id, p_btif);
                break;
            }
        }        
    }
    if(NULL == p_btif)
    {
        BTIF_INFO_FUNC("no btif structure found for BTIF's user id(0x%08x)\n", u_id);
    }
    return p_btif;
}
...
* PARAMETERS
*  p_owner      [IN] pointer to owner who call this API, currently there are 2 owner ("stp" or "btif_tester") may use this module
...
*                   if open success, value p_id will be the only identifier for user to access BTIF's other operations 
...
int mtk_wcn_btif_close(unsigned int u_id)
{
    ...
    list_for_each(pos, p_user_list)
    {
        p_mtk_btif_user p_user = container_of(pos, mtk_btif_user, entry);
        if (p_user->u_id == u_id)
        {
            BTIF_INFO_FUNC("user who's id is 0x%08x deleted from user list\n", u_id);
            list_del(pos);
            vfree(p_user);
...
*      if btif driver detected that no space is available in Tx FIFO, will return E_BTIF_NO_SPACE, mostly something is wrong with BTIF or consys when this return value is returned
* RETURNS
*  int          positive: data length send through BTIF; negative: please see ENUM_BTIF_OP_ERROR_CODE
*        E_BTIF_AGAIN (0) will be returned to caller if btif does not have enough vFIFO to send data, when caller get 0, he should wait for a moment (5~10ms maybe) and try a few times (maybe 10~20)
*            if still get E_BTIF_AGAIN, should call BTIF's debug API and dump BTIF driver and BTIF/DMA register information to kernel log for debug
*        E_BTIF_BAD_POINTER will be returned to caller if btif is not opened successfully before call this API
*        E_BTIF_INVAL_PARAM will be returned if parameter is not valid

*****************************************************************************/
int mtk_wcn_btif_write(unsigned int u_id, const unsigned char *p_buf, unsigned int len)
{
    int i_ret = -1;
    p_mtk_btif p_btif = NULL;
...
* RETURNS
*  int          positive: data length read from BTIF; negative: please see ENUM_BTIF_OP_ERROR_CODE
*****************************************************************************/
int mtk_wcn_btif_read(unsigned int u_id, unsigned char *p_buf, unsigned int max_len)
{
    return 0;
}
...
* PARAMETERS
*  p_btif      [IN] pointer returned by mtk_wcn_btif_open
*  rx_cb       [IN] pointer to stp rx handler callback function, should be comply with MTK_WCN_BTIF_RX_CB
* RETURNS
*  int          0 = succeed; others = fail, for detailed information, please see ENUM_BTIF_OP_ERROR_CODE
*****************************************************************************/
...
* DESCRIPTION
*  once sleep command is sent to con sys, should call this API before send wakeup command to make con sys aware host want to send data to consys
* PARAMETERS
*  p_btif      [IN] pointer returned by mtk_wcn_btif_open
* RETURNS
*  int          0 = succeed; others = fail, for detailed information, please see ENUM_BTIF_OP_ERROR_CODE
...
* PARAMETERS
*  p_btif      [IN] pointer returned by mtk_wcn_btif_open
*  enable     [IN] loopback mode control flag, enable or disable, shou be one of ENUM_BTIF_LPBK_MODE
* RETURNS
*  int          0 = succeed; others = fail, for detailed information, please see ENUM_BTIF_OP_ERROR_CODE
*****************************************************************************/
int mtk_wcn_btif_loopback_ctrl(unsigned int u_id, ENUM_BTIF_LPBK_MODE enable)
{
    ...
    //i_ret = hal_btif_loopback_ctrl(p_btif->p_btif_info, enable == BTIF_LPBK_ENABLE ? true : false);
    i_ret = btif_lpbk_ctrl(p_btif, enable == BTIF_LPBK_ENABLE ? true : false);
...
int mtk_wcn_btif_dbg_ctrl(unsigned int u_id, ENUM_BTIF_DBG_ID flag)
{
    ...
    switch (flag)
    {
        case BTIF_DISABLE_LOGGER:
            {
                BTIF_INFO_FUNC("disable btif log function for both Tx and Rx\n");
        ...
        case BTIF_ENABLE_LOGGER:
            {
                BTIF_INFO_FUNC("enable btif log function for both Tx and Rx\n");
        ...
        case BTIF_ENABLE_RT_LOG:
            BTIF_INFO_FUNC("enable btif real time log for both Tx and Rx\n");
            btif_log_output_enable(&p_btif->tx_log);
            btif_log_output_enable(&p_btif->rx_log);
            break;
        case BTIF_DISABLE_RT_LOG:
            BTIF_INFO_FUNC("disable btif real time log for both Tx and Rx\n");
...
//      p_buf[idx] = BUF_LEN -idx;
    ...
    while (loop--)
    {
        ...
        if (i_ret != buf_len)
        {
            BTIF_INFO_FUNC("mtk_wcn_btif_write failed, target len %d, sent len: %d\n", buf_len, i_ret);
            break;
        }
        buf_len--;
        if (0 >= buf_len)
