...
extern int mtk_btif_dbg_lvl;

#define BTIF_LOUD_FUNC(fmt, arg...) \
    do {\
        if (mtk_btif_dbg_lvl >= BTIF_LOG_LOUD){ \
            hal_log_print(DFT_TAG "[L]%s:"  fmt, __FUNCTION__ ,##arg);\
        }\
    }while(0)
                                           
#define BTIF_INFO_FUNC(fmt, arg...) \
    do {\
        if (mtk_btif_dbg_lvl >= BTIF_LOG_INFO) { \
            hal_log_print(DFT_TAG "[I]%s:"  fmt, __FUNCTION__ ,##arg); \
        }\
    } while (0)
    
#define BTIF_WARN_FUNC(fmt, arg...) \
    do {\
        if (mtk_btif_dbg_lvl >= BTIF_LOG_WARN) { \
            hal_log_print(DFT_TAG "[W]%s:"  fmt, __FUNCTION__ ,##arg); \
        } \
    } while (0)
    
#define BTIF_ERR_FUNC(fmt, arg...)\
    do {\
        if (mtk_btif_dbg_lvl >= BTIF_LOG_ERR) {\
            hal_log_print(DFT_TAG "[E]%s(%d):"  fmt, __FUNCTION__ , __LINE__, ##arg);\
        }\
    } while (0)
    
#define BTIF_DBG_FUNC(fmt, arg...)\
    do {\
        if (mtk_btif_dbg_lvl >= BTIF_LOG_DBG) {\
            hal_log_print(DFT_TAG "[D]%s:"  fmt, __FUNCTION__ ,##arg);\
        }\
    } while (0)
...
typedef struct _DMA_VFIFO_ {
    unsigned char  *p_vir_addr;        /*[Driver Access] vFIFO memory'svirtual address*/
    dma_addr_t     phy_addr;           /*[HW Access] dma handle , physically address, set to DMA's HW Register*/
    unsigned int   vfifo_size;     /*DMA's vFIFO size*/
    unsigned int   thre;           /*DMA's threshold value*/
}DMA_VFIFO, *P_DMA_VFIFO;

typedef unsigned int (*dma_rx_buf_write)(/*P_MTK_DMA_INFO_STR*/ void *p_dma_info, unsigned char* p_buf, unsigned int buf_len);
typedef unsigned int (*btif_rx_buf_write)(/*P_MTK_BTIF_INFO_STR*/ void * p_btif_info, unsigned char* p_buf, unsigned int buf_len);
...
/******************************register operation ********************************/
#define btif_reg_sync_writeb(v, a)    mt65xx_reg_sync_writeb((v), (a)) /*byte write  <1 byte>*/
#define btif_reg_sync_writew(v, a)    mt65xx_reg_sync_writew((v), (a)) /*word write  <2 byte>*/
#define btif_reg_sync_writel(v, a)    mt65xx_reg_sync_writel((v), (a)) /*long write   <4 byte>*/

#define BTIF_READ8(REG)               __raw_readb(REG)
#define BTIF_READ16(REG)              __raw_readw(REG)
#define BTIF_READ32(REG)              __raw_readl(REG)


#define BTIF_SET_BIT(REG, BITVAL)    do{*((volatile unsigned int *)(REG)) |= ((unsigned int)(BITVAL)); mb();} while(0)
#define BTIF_CLR_BIT(REG, BITVAL)    do{(*(volatile unsigned int *)(REG)) &= ~((unsigned int)(BITVAL)); mb();} while(0)
