...
static ssize_t btif_file_read (struct file *pfile, char __user* buf, size_t count, loff_t *f_ops);
unsigned int btif_poll(struct file *filp, poll_table *wait);
int _btif_irq_reg (P_MTK_BTIF_IRQ_STR p_irq, mtk_btif_irq_handler irq_handler, void *data);
...
unsigned int btif_pio_rx_data_receiver(P_MTK_BTIF_INFO_STR p_btif_info, unsigned char* p_buf, unsigned int buf_len);

irqreturn_t btif_tx_dma_irq_handler(int irq, void *data);
irqreturn_t btif_rx_dma_irq_handler(int irq, void *data);

unsigned int btif_dma_rx_data_receiver(P_MTK_DMA_INFO_STR p_dma_info, unsigned char* p_buf, unsigned int buf_len);
...
int _btif_pio_write(p_mtk_btif p_btif, const unsigned char* p_buf, unsigned int buf_len);
int _btif_dma_write(p_mtk_btif p_btif, const unsigned char* p_buf, unsigned int buf_len);


unsigned int btif_bbs_wr_direct(p_btif_buf_str p_bbs, unsigned char* p_buf, unsigned int buf_len);
unsigned int btif_bbs_read(p_btif_buf_str p_bbs, unsigned char* p_buf, unsigned int buf_len);
unsigned int btif_bbs_write(p_btif_buf_str p_bbs, unsigned char* p_buf, unsigned int buf_len);
void btif_dump_bbs_str(unsigned char *p_str, p_btif_buf_str p_bbs);
void btif_bbs_wr_byte_chk(p_btif_buf_str p_bbs, unsigned char val);
...
static int _btif_dpidle_notify_ctrl(p_mtk_btif p_btif, ENUM_BTIF_DPIDLE_CTRL en_flag);
...
int _btif_send_data(p_mtk_btif p_btif, const unsigned char* p_buf, unsigned int buf_len);
...
static struct dev_pm_ops mtk_btif_drv_pm_ops = {
    .restore_noirq = mtk_btif_restore_noirq,
    .suspend = mtk_btif_drv_suspend,
    .resume  = mtk_btif_drv_resume,
};

static struct platform_driver mtk_btif_dev_drv =
{
...
int _btif_suspend(p_mtk_btif p_btif)
{
    ...
    if (NULL != p_btif)
    {
        ...
        {
            /*before disable BTIF controller and DMA controller we need to set BTIF to ON state*/
...
int _btif_restore_noirq (p_mtk_btif p_btif)
{
	...
	{
	    i_ret = hal_dma_pm_ops(p_btif->p_tx_dma->p_dma_info, BTIF_PM_RESTORE_NOIRQ);
		if (0 == i_ret)
		{
		...
		{
		    BTIF_INFO_FUNC("BTIF Tx DMA IRQ restore failed, i_ret:%d\n", i_ret);
			return i_ret;
		}
	}
	if (p_btif->rx_mode & BTIF_MODE_DMA)
	{
	    i_ret = hal_dma_pm_ops(p_btif->p_rx_dma->p_dma_info, BTIF_PM_RESTORE_NOIRQ);
		if (0 == i_ret)
		{
		...
		{
		    BTIF_INFO_FUNC("BTIF Rx DMA IRQ restore failed, i_ret:%d\n", i_ret);
			return i_ret;
		}
	}
	return i_ret;
...
dev_t btif_dev;
struct class *p_btif_class = NULL;
struct device *p_btif_dev = NULL;
...
static int rx_notify_flag = 0;
static DECLARE_WAIT_QUEUE_HEAD(btif_wq);
static int btif_file_open (struct inode *pinode, struct file *pfile);
static int btif_file_release (struct inode *pinode, struct file *pfile);
static ssize_t btif_file_read (struct file *pfile, char __user* buf, size_t count, loff_t *f_ops);

ssize_t btif_file_write(struct file *filp,  const char __user *buf, size_t count, loff_t *f_pos);
long btif_unlocked_ioctl ( struct file *filp, unsigned int cmd, unsigned long arg);

struct cdev btif_dev_c;
static wait_queue_head_t btif_read_inq;    /* read queues */

struct file_operations mtk_btif_fops = 
{
	.owner = THIS_MODULE,
	.open = btif_file_open,
...
static int btif_chrdev_init(void)
{
    ...
    i_ret = alloc_chrdev_region(&btif_dev, 0, 1, p_btif_dev_name); // alloc device number dynamically
    if (i_ret)
    {
    ...
    p_btif_dev = device_create(p_btif_class, NULL, btif_dev, 0, p_btif_dev_name);
...
static int btif_file_release (struct inode *pinode, 
    struct file *pfile)
{    
    int i_ret = -1;
    BTIF_INFO_FUNC("pid:%d\n", current->pid);
...
static ssize_t driver_flag_set(struct device_driver *drv, const char *buffer, size_t count)
{
    char buf[256];
    char *pBuf;
    ...
    char *pToken = NULL;
    char *pDelimiter = " \t";
    BTIF_INFO_FUNC("buffer = %s, count = %d\n", buffer, count);
    if(len >= sizeof(buf)){
...
int _btif_irq_reg (P_MTK_BTIF_IRQ_STR p_irq, mtk_btif_irq_handler irq_handler, void *data)
{
    ...
    i_ret = request_irq(irq_id, (irq_handler_t)irq_handler, flag, p_irq->name, data);
    if (i_ret)
    {
        return i_ret;
    }
...
int _btif_controller_tx_setup(p_mtk_btif p_btif)
{
    ...
    if (BTIF_MODE_DMA == p_btif->tx_mode)
    {
        ...
        if (i_ret)
        {
            BTIF_ERR_FUNC("_btif_tx_dma_setup failed, i_ret(%d), set tx to PIO mode\n", i_ret);
...
int _btif_rx_dma_setup(p_mtk_btif p_btif)
{
    ...
    if (i_ret)
    {
        BTIF_ERR_FUNC("hal_btif_dma_clk_ctrl failed, i_ret(%d), set rx to pio mode\n", i_ret);
    ...
    hal_btif_dma_rx_cb_reg(p_dma_info, (dma_rx_buf_write)btif_dma_rx_data_receiver);
    
    /*DMA controller enable*/
    i_ret = hal_btif_dma_ctrl(p_dma_info, DMA_CTRL_ENABLE);
    if (i_ret)
    {
        BTIF_ERR_FUNC("hal_btif_dma_ctrl failed, i_ret(%d), set rx to pio mode\n", i_ret);
        hal_btif_dma_clk_ctrl(p_dma_info, CLK_OUT_DISABLE);
        /*DMA control failed set Rx to PIO mode*/
        i_ret = _btif_rx_pio_setup(p_btif);
    }
...
int _btif_tx_dma_setup(p_mtk_btif p_btif)
{
    ...
    i_ret = hal_btif_dma_ctrl(p_dma_info, DMA_CTRL_ENABLE);
    if (i_ret){
        BTIF_ERR_FUNC("hal_btif_dma_ctrl failed, i_ret(%d), set tx to pio mode\n", i_ret);
...
int _btif_controller_setup(p_mtk_btif p_btif)
{
    ...
    /************************************************/
    hal_btif_rx_cb_reg(p_btif_info, (btif_rx_buf_write)btif_pio_rx_data_receiver);
...
int _btif_exit_dpidle_from_dpidle(p_mtk_btif p_btif)
{
    ...
    if (p_btif->tx_mode == BTIF_MODE_DMA)
    {
        ...
        i_ret += hal_btif_dma_clk_ctrl(p_btif->p_tx_dma->p_dma_info, CLK_OUT_ENABLE);
    }
    if (p_btif->rx_mode == BTIF_MODE_DMA)
    {
        /*enable BTIF Rx DMA's clock*/
        i_ret += hal_btif_dma_clk_ctrl(p_btif->p_rx_dma->p_dma_info, CLK_OUT_ENABLE);
...
{
#define MAX_WAIT_TIME 60000 /*means 1 minitute*/
    int i_ret = 0;
    unsigned int retry = 0;
    unsigned int wait_period = 1;
    unsigned int max_retry = MAX_WAIT_TIME / wait_period;
    ...
    if (retry < max_retry)
    {
        if (p_btif->tx_mode == BTIF_MODE_DMA)
        {
            ...
            i_ret += hal_btif_dma_clk_ctrl(p_btif->p_tx_dma->p_dma_info, CLK_OUT_DISABLE);
        }
        if (p_btif->rx_mode == BTIF_MODE_DMA)
        {
            /*disable BTIF Rx DMA's clock*/
            i_ret += hal_btif_dma_clk_ctrl(p_btif->p_rx_dma->p_dma_info, CLK_OUT_DISABLE);
            
        }
        /*disable BTIF's clock*/
        i_ret += hal_btif_clk_ctrl(p_btif->p_btif_info, CLK_OUT_DISABLE);
...
int btif_rx_cb_reg(p_mtk_btif p_btif, MTK_WCN_BTIF_RX_CB rx_cb)
{
    if (p_btif->rx_cb)
    {
        BTIF_WARN_FUNC("rx cb already exist, rewrite from (0x%x) to (0x%x)\n", p_btif->rx_cb, rx_cb);
    }
    p_btif->rx_cb = rx_cb;
...
static int _btif_vfifo_init(p_mtk_btif_dma p_dma)
{
    ...
    {
        BTIF_ERR_FUNC("BTIF vFIFO memory already allocated, do nothing\n");
        return E_BTIF_BAD_POINTER;
    }
    
    /*vFIFO memory allocation*/
    p_vfifo->p_vir_addr = dma_zalloc_coherent(NULL, p_vfifo->vfifo_size, &p_vfifo->phy_addr, GFP_DMA);
    if (NULL == p_vfifo->p_vir_addr)
    {
        BTIF_ERR_FUNC("alloc vFIFO memory for BTIF failed\n");
        return E_BTIF_FAIL;
    }
    BTIF_INFO_FUNC("alloc vFIFO memory for BTIF succeed, vir addr:0x%08x, phy addr:0x%08x\n", p_vfifo->p_vir_addr, p_vfifo->phy_addr);
    return 0;
}
...
static int _btif_vfifo_deinit(p_mtk_btif_dma p_dma)
{
    ...
    if (NULL != p_vfifo->p_vir_addr) 
    {
        dma_free_coherent(NULL, p_vfifo->vfifo_size, p_vfifo->p_vir_addr, p_vfifo->phy_addr);
...
static int _btif_state_set(p_mtk_btif p_btif, ENUM_BTIF_STATE state)
{
    ...
    {
        BTIF_DBG_FUNC("%s->%s request\n", g_state[ori_state], g_state[state]);
        if (B_S_ON == state)
            _btif_dpidle_notify_ctrl(p_btif, BTIF_DPIDLE_DISABLE);
        switch(ori_state)
        {
            case B_S_ON:
                ...
                /*B_S_ON->B_S_DPLE : disable clock backup BTIF and DMA controller's register if necessary*/
                if (B_S_DPIDLE == state)
                {
            ...
            case B_S_DPIDLE:
                ...
                /*B_S_DPIDLE-> B_S_SUSPEND: disable clock backup BTIF and DMA controller's register if necessary*/
                if (B_S_ON == state)
                {
            ...
            case B_S_SUSPEND:
                ...
                if (B_S_DPIDLE == state)
                {
                    /*system call resume API, do resume operation, change to deepidle state*/
                    p_btif->state = state;
                    i_ret =0;
                    
                }else if (B_S_ON == state){
                    /*when stp want to send data before system do resume operation*/
                    p_btif->state = state;
                    i_ret =0;
                }else{
                    BTIF_ERR_FUNC("%s->%s is not allowed\n", g_state[ori_state], g_state[state]);
            ...
            default:
                /*no this possibility*/
                BTIF_ERR_FUNC("state change request is not allowed, this should never happen\n");
                break;
        }
...
static int btif_rx_data_consummer (p_mtk_btif p_btif)
{
    ...
    do {
        if (0 < length)
        {
            /*check if rx_cb empty or not, if registered , call user's rx callback to handle these data*/
            if (p_btif->rx_cb)
            {
            ...
            {
                BTIF_WARN_FUNC("p_btif:0x%08x, both rx_notify and rx_cb are NULL\n", p_btif);
                break;
            }
        }
        else
...
static void btif_tx_worker (struct work_struct *p_work)
{
    ...
    BTIF_MUTEX_LOCK(&(p_btif->tx_mtx));
    how_much_get= kfifo_out(p_btif->p_tx_fifo, local_buf, sizeof(local_buf));
    do {
        while (leng_sent < how_much_get)
        {
            i_ret = _btif_send_data(p_btif, local_buf + leng_sent, how_much_get - leng_sent);
            if (0 < i_ret)
            {
                leng_sent += i_ret;
            }
            ...
            {
                 BTIF_WARN_FUNC("_btif_send_data return 0, retry\n");
            }
            else
            {
                 BTIF_WARN_FUNC("!!!!!!<<<<<btif send data fail,reset tx fifo, i_ret(%d)>>>>>!!!!!!\n", i_ret);
...
static int _btif_tx_ctx_init(p_mtk_btif p_btif)
{
    ...
    if (BTIF_TX_SINGLE_CTX == p_btif->tx_ctx)
    {
        ...
        if (!(p_btif->p_tx_wq)) {
            BTIF_ERR_FUNC("create_singlethread_workqueue for tx thread fail\n");
            i_ret = -ENOMEM;
            goto btm_init_err;
        }
        ...
        i_ret = kfifo_alloc(p_btif->p_tx_fifo, BTIF_TX_FIFO_SIZE, GFP_ATOMIC);
        if (0 != i_ret){
    ...
    else if (BTIF_TX_USER_CTX == p_btif->tx_ctx)
    {
        ...
        BTIF_INFO_FUNC("nothing is done when btif tx in user's thread\n");
    }else
    {
...
static int _btif_rx_btm_init(p_mtk_btif p_btif)
{
    ...
    if (BTIF_THREAD_CTX == p_btif->btm_type)
    {
        ...
        p_btif->p_rx_wq = create_singlethread_workqueue("btif_rxd");
        if (!(p_btif->p_rx_wq)) {
    ...
    {
        /*init rx tasklet*/
        tasklet_init(&(p_btif->rx_tasklet), btif_rx_tasklet, (unsigned long)p_btif);
...
void btif_bbs_wr_byte_chk(p_btif_buf_str p_bbs, unsigned char val)
{
    /*Here we check if available size , left 1 bit to prevent from read index and write index are equal to each other*/
    if (BBS_AVL_SIZE(p_bbs) > 1)
    {
        (p_bbs)->buf[(p_bbs)->wr_idx & BBS_MASK(p_bbs)] = val;
        BBS_WRITE_MOVE_NEXT(p_bbs);
    }
    else
    {
        BTIF_ERR_FUNC ("BTIF overrun, val(0x%B)\n", val);
    }
}

void btif_dump_bbs_str(unsigned char *p_str, p_btif_buf_str p_bbs)
{
    BTIF_INFO_FUNC("%s UBS:0x%08x\n  Size:0x%08x\n  read:0x%08x\n  write:0x%08x\n",p_str,  p_bbs, p_bbs->size, p_bbs->rd_idx, p_bbs->wr_idx);
    return;
}
...
unsigned int btif_bbs_write(p_btif_buf_str p_bbs, unsigned char* p_buf, unsigned int buf_len)
{
    ...
    unsigned int ava_len = emp_len - 1;
     ...
     if (0 >= ava_len)
     {
         BTIF_ERR_FUNC("no empty space left for write, (%d)ava_len, (%d)to write\n", ava_len, buf_len);
         return 0;
     }
...
//    BTIF_Rx_IRQ_Disable();    
    ...
    //opt_print("%s:ava_len(%d), rdIdx(%d), wrIdx(%d)\n",__func__ , ava_len, p_bbs->rd_idx, p_bbs->wr_idx);
    if (ava_len >= 4096)
    {
    ...
    }else if (buf_len >= ava_len)
    {
        ...
        if (4096 <= rd_len)
        {
            btif_dump_bbs_str("rd_len > 4096", p_bbs);
        }
        #endif
        if (wr_idx >= (p_bbs)->rd_idx)
        {
            memcpy(p_buf, BBS_PTR(p_bbs, p_bbs->rd_idx)/*&(p_bbs)->buf[(p_bbs)->rd_idx]*/, ava_len);
            (p_bbs)->rd_idx = wr_idx;
        }
        else
        {
            unsigned int tail_len = BBS_SIZE(p_bbs) - (p_bbs)->rd_idx;
            memcpy(p_buf, BBS_PTR(p_bbs, p_bbs->rd_idx)/*&(p_bbs)->buf[(p_bbs)->rd_idx]*/, tail_len);
            memcpy(p_buf + tail_len,BBS_PTR(p_bbs, 0)/* &(p_bbs)->buf[0]*/, ava_len - tail_len);
            (p_bbs)->rd_idx = wr_idx;
        }
    }
    else
    {
        ...
        if (4096 <= rd_len)
        {
            BTIF_dump_ubs_str("rd_len > 4096", p_bbs);
        }
        #endif
        if (wr_idx >= (p_bbs)->rd_idx)
        {
            memcpy(p_buf, BBS_PTR(p_bbs, p_bbs->rd_idx)/*&(p_bbs)->buf[(p_bbs)->rd_idx]*/, rd_len);
        ...
        else
        {
            ...
            {
                memcpy(p_buf, BBS_PTR(p_bbs, p_bbs->rd_idx)/*&(p_bbs)->buf[(p_bbs)->rd_idx]*/, rd_len);
                (p_bbs)->rd_idx = ((p_bbs)->rd_idx + rd_len) & (BBS_MASK(p_bbs));
            }
            else
            {
                memcpy(p_buf, BBS_PTR(p_bbs, p_bbs->rd_idx)/*&(p_bbs)->buf[(p_bbs)->rd_idx]*/, tail_len);
                memcpy(p_buf + tail_len, &(p_bbs)->buf[0], rd_len - tail_len);
...
int _btif_dma_write(p_mtk_btif p_btif, const unsigned char* p_buf, unsigned int buf_len)
{
    ...
    do {
        ...
        while (!hal_dma_is_tx_allow(p_dma_info) && (retry < max_tx_retry))
        {
            retry++;
...
int _btif_pio_write(p_mtk_btif p_btif, const unsigned char* p_buf, unsigned int buf_len)
{
    ...
    while ((sent_len < buf_len))
    {
        if(hal_btif_is_tx_allow(p_btif_info)){
            i_ret = hal_btif_send_data(p_btif_info, p_buf + sent_len, buf_len - sent_len);
            if (0 < i_ret)
            {
              sent_len += i_ret;
...
int btif_send_data(p_mtk_btif p_btif, const unsigned char* p_buf, unsigned int buf_len)
{
    int i_ret = 0;
    ...
    }else if (BTIF_TX_SINGLE_CTX == p_btif->tx_ctx)
    {
        int length = 0;
        /*tx fifo in*/
        length = kfifo_in(p_btif->p_tx_fifo, (unsigned char *)p_buf, buf_len);
        if (length == buf_len)
        {
...
int btif_dump_reg(p_mtk_btif p_btif)
{
    ...
    {
        i_ret = E_BTIF_INVAL_STATE;
        BTIF_ERR_FUNC("BTIF in OFF state, should no need to dump register, please check wmt's operation is okay or not.\n");
        goto dmp_reg_err;
    }
...
int btif_log_buf_dmp_in(P_BTIF_LOG_QUEUE_T p_log_que, const char *p_buf, int len)
{
    ...
    {
        BTIF_ERR_FUNC("invalid parameter, p_log_que(0x%x),  buf(0x%x),  len(%d)\n", p_log_que, p_buf, len);
        return 0;
    }
    ...
    p_log_que->size++;
    p_log_que->size = p_log_que->size > BTIF_LOG_ENTRY_NUM ? BTIF_LOG_ENTRY_NUM : p_log_que->size;
...
int btif_log_buf_dmp_out(P_BTIF_LOG_QUEUE_T p_log_que)
{
    ...
    out_index = p_log_que->size >=  BTIF_LOG_ENTRY_NUM ? in_index : (BTIF_LOG_ENTRY_NUM - p_log_que->size + in_index) % BTIF_LOG_ENTRY_NUM;
    ...
    if (0 != dump_size)
    {
        ...
        while (dump_size--)
        {
            ...
            printk("dir:%s, pkt_count:%d, %d.%ds len:%d\n", p_dir, pkt_count++, (int)p_timer->tv_sec, (int)p_timer->tv_usec,len);
            /*output buffer content*/
            btif_dump_data(p_log_buf->buffer, len > BTIF_LOG_SZ ? BTIF_LOG_SZ : len);
            out_index++;
...
/*SW init*/
    for (index = 0; index < BTIF_PORT_NR; index++)
    {
        ...
        i_ret = hal_btif_clk_ctrl(g_btif[index].p_btif_info, CLK_OUT_DISABLE);
        if (0 != i_ret)
        {
    ...
    /*viftual FIFO memory must be physical continious, because DMA will access it directly without MMU*/
        ...
        i_ret = hal_btif_dma_clk_ctrl(p_tx_dma->p_dma_info, CLK_OUT_DISABLE);
        if (0 != i_ret)
        {
...
#if !(MTK_BTIF_ENABLE_CLK_REF_COUNTER)
        /*enable BTIF Tx DMA channel's clock gating by default*/
        i_ret = hal_btif_dma_clk_ctrl(p_rx_dma->p_dma_info, CLK_OUT_DISABLE);
        if (0 != i_ret)
        {
