...
{
    disp_drv_dbg_log = enable;
    disp_drv_dbg_info_log = enable;
    disp_drv_dbg_func_log = enable;
    disp_drv_dbg_upd_log = enable;
    DISP_DRV_INFO("disp common log %s\n", enable?"enabled":"disabled");
}
// ---------------------------------------------------------------------------
//  Local Functions
// ---------------------------------------------------------------------------
...
static void lcm_set_reset_pin(UINT32 value)
{
    ...
    {
        ret = enable_clock(MT_CG_DISP_DBI_ENGINE_SW_CG, "DSI_LRST");
    }
    LCD_SetResetSignal(value);
    if (LCM_TYPE_DSI == lcm_params->type)
    {
        ret += disable_clock(MT_CG_DISP_DBI_ENGINE_SW_CG, "DSI_LRST");
    }
    if(ret > 0)
    {
...
static void disp_drv_init_ctrl_if(void)
{
    ...
    if((lcm_params== NULL) || (is_lcm_inited == TRUE))
    {
        return;
    }
...
DISP_STATUS DISP_Init(UINT32 fbVA, UINT32 fbPA, BOOL isLcmInited)
{
    ...
    {
        DISP_DRV_WRAN("DISP_InitVSYNC(): Cannot create capture fb kthread\n");
    }
    ...
    if (lcm_params)
    {
        ...
        {
            OUTREG32(DISPSYS_CONFIG_BASE + 0xC08, 0x200);
        }
        else if (lcm_params->type==LCM_TYPE_DPI)
        {
            OUTREG32(DISPSYS_CONFIG_BASE + 0xC08, 0x201);
        }
    }
#endif
...
DISP_STATUS DISP_Set3DPWM(BOOL enable, BOOL landscape)
{
    ...
    if (enable && (!ispwmenabled || ismodechanged))
    {
        ...
        DISP_DRV_INFO("3D PWM is enabled. landscape:%d ! \n", landscape);
    }
    ...
    {	
        ...
        DISP_DRV_INFO("3D PWM is disabled ! \n");
    }
#endif
    return DISP_STATUS_OK;
}
...
DISP_STATUS DISP_UpdateScreen(UINT32 x, UINT32 y, UINT32 width, UINT32 height)
{
    ...
    DISP_DRV_INFO("update screen, (%d,%d),(%d,%d)\n", x, y, width, height);
...
static void _DISP_InterruptCallbackProxy(DISP_INTERRUPT_EVENTS eventID)
{
    ...
    {
        DISP_DRV_WRAN("Invalid event id: %d\n", eventID);
        ASSERT(0);
    }
...
static int _DISP_CaptureOvlKThread(void *data)
{
    ...
    while(1)
    {
        ...
        if (init == 0)
        {
			...
			m4u_alloc_mva(M4U_CLNTMOD_DISP, (unsigned int)va[0], buf_size, 0, 0, &(mva[0]));
			m4u_alloc_mva(M4U_CLNTMOD_DISP, (unsigned int)va[1], buf_size, 0, 0, &(mva[1]));
            disp_power_on(DISP_MODULE_WDMA0, &power_on_record);
        ...
        enabled = 1;
#ifndef CONFIG_ARCH_MT6582
        m4u_dma_cache_maint(M4U_CLNTMOD_DISP,
                                            va[index],
...
void DISP_InitVSYNC(unsigned int vsync_interval)
{
    ...
    {
        DISP_DRV_WRAN("DISP_FMDesense_Query():unknown interface\n");
        ASSERT(0);
    }
#else
    ...
    {
        DISP_DRV_WRAN("DISP_InitVSYNC(): Cannot create config update kthread\n");
        return;
    }
...
//            DISP_SetInterruptCallback(DISP_DSI_VSYNC_INT, &cb);
    ...
    {
        DISP_DRV_WRAN("DISP_InitVSYNC():unknown interface\n");
        ASSERT(0);
    }
#endif
}
...
void DISP_WaitVSYNC(void)
{
    ...
    {
        DISP_DRV_WRAN("DISP_WaitVSYNC():unknown interface\n");
        ASSERT(0);
    }
#else
    if((LCM_TYPE_DBI == lcm_params->type))
...
DISP_STATUS DISP_PauseVsync(BOOL enable)
{
    if((LCM_TYPE_DBI == lcm_params->type) || 
        ...
        if (enable)
        {
            ...
            DISP_DRV_UPD("[FB PV] 1\n");
        }
        else
        {
            hrtimer_start(&cmd_mode_update_timer, cmd_mode_update_timer_period, HRTIMER_MODE_REL);
            DISP_DRV_UPD("[FB PV] 2\n");
        }
    ...
    {
        DISP_DRV_WRAN("DISP_PauseVSYNC():unknown interface\n");
        ASSERT(0);
    }
    return DISP_STATUS_OK;
}
...
{
    DISP_DRV_INFO("DISP_ConfigDither lrs:0x%x, lgs:0x%x, lbs:0x%x, dbr:0x%x, dbg:0x%x, dbb:0x%x \n", lrs, lgs, lbs, dbr, dbg, dbb);
    ...
    {
        DISP_DRV_WRAN("DISP_ConfigDither error \n");
        return DISP_STATUS_ERROR;
    }
}
...
DISP_STATUS DISP_SetScreenBpp(UINT32 bpp)
{
    ...
    if( bpp != 16   &&          \
        ...
        1 )
    {
        DISP_DRV_WRAN("DISP_SetScreenBpp error, not support %d bpp\n", bpp);
        return DISP_STATUS_ERROR;
    }

    disp_fb_bpp = bpp;
    DISP_DRV_INFO("DISP_SetScreenBpp %d bpp\n", bpp);
...
DISP_STATUS DISP_SetPages(UINT32 pages)
{
    ...
    disp_fb_pages = pages;
    DISP_DRV_INFO("DISP_SetPages %d pages\n", pages);
...
UINT32 DISP_GetVRamSize(void)
{
    ...
    if (0 == vramSize)
    {
        ...
        DISP_DRV_INFO("DISP_GetVRamSize: %u bytes\n", vramSize);
    }
...
#ifndef CONFIG_ARCH_MT6582
#ifndef CONFIG_ARCH_MT6589
    ret = m4u_alloc_mva(M4U_CLNTMOD_DISP, 
        pvbuf, 
...
DISP_STATUS DISP_FM_Desense(unsigned long freq)
{
    ...
    if (down_interruptible(&sem_update_screen)) {
        DISP_DRV_WRAN("ERROR: Can't get sem_update_screen in DISP_FM_Desense()\n");
        return DISP_STATUS_ERROR;
    }
    
    if(LCM_TYPE_DBI == lcm_params->type){//DBI
        DISP_DRV_INFO("DISP_FM_Desense():DBI interface\n");        
        LCD_CHECK_RET(LCD_FM_Desense(ctrl_if, freq));
    }
    else if(LCM_TYPE_DPI == lcm_params->type){//DPI
        DISP_DRV_INFO("DISP_FM_Desense():DPI interface\n");
        DPI_CHECK_RET(DPI_FM_Desense(freq));
    }
    else if(LCM_TYPE_DSI == lcm_params->type){// DSI
        DISP_DRV_INFO("DISP_FM_Desense():DSI interface\n");
        DSI_CHECK_RET(DSI_FM_Desense(freq));
    }
    else
    {
        DISP_DRV_WRAN("DISP_FM_Desense():unknown interface\n");
...
DISP_STATUS DISP_Reset_Update()
{
    ...
    if (down_interruptible(&sem_update_screen)) {
        DISP_DRV_WRAN("ERROR: Can't get sem_update_screen in DISP_Reset_Update()\n");
        return DISP_STATUS_ERROR;
    }
    
    if(LCM_TYPE_DBI == lcm_params->type){//DBI
        DISP_DRV_INFO("DISP_Reset_Update():DBI interface\n");        
        LCD_CHECK_RET(LCD_Reset_WriteCycle(ctrl_if));
    }
    else if(LCM_TYPE_DPI == lcm_params->type){//DPI
        DISP_DRV_INFO("DISP_Reset_Update():DPI interface\n");
        DPI_CHECK_RET(DPI_Reset_CLK());
    }
    else if(LCM_TYPE_DSI == lcm_params->type){// DSI
        DISP_DRV_INFO("DISP_Reset_Update():DSI interface\n");
        DSI_CHECK_RET(DSI_Reset_CLK());
    }
    else
    {
        DISP_DRV_WRAN("DISP_Reset_Update():unknown interface\n");
        ret = DISP_STATUS_ERROR;
    }
...
DISP_STATUS DISP_Get_Default_UpdateSpeed(unsigned int *speed)
{
    ...
    if(LCM_TYPE_DBI == lcm_params->type){//DBI
        DISP_DRV_INFO("DISP_Get_Default_UpdateSpeed():DBI interface\n");        
        LCD_CHECK_RET(LCD_Get_Default_WriteCycle(ctrl_if, speed));
    }
    else if(LCM_TYPE_DPI == lcm_params->type){//DPI
        DISP_DRV_INFO("DISP_Get_Default_UpdateSpeed():DPI interface\n");
        DPI_CHECK_RET(DPI_Get_Default_CLK(speed));
    }
    else  if(LCM_TYPE_DSI == lcm_params->type){// DSI
        DISP_DRV_INFO("DISP_Get_Default_UpdateSpeed():DSI interface\n");
...
DISP_STATUS DISP_Get_Current_UpdateSpeed(unsigned int *speed)
{
    ...
    if(LCM_TYPE_DBI == lcm_params->type){//DBI
        DISP_DRV_INFO("DISP_Get_Current_UpdateSpeed():DBI interface\n");        
        LCD_CHECK_RET(LCD_Get_Current_WriteCycle(ctrl_if, speed));
    }
    else if(LCM_TYPE_DPI == lcm_params->type){//DPI
        DISP_DRV_INFO("DISP_Get_Current_UpdateSpeed():DPI interface\n");
        DPI_CHECK_RET(DPI_Get_Current_CLK(speed));
    }
    else if(LCM_TYPE_DSI == lcm_params->type){// DSI
        DISP_DRV_INFO("DISP_Get_Current_UpdateSpeed():DSI interface\n");
...
DISP_STATUS DISP_Change_Update(unsigned int speed)
{
    ...
    if(LCM_TYPE_DBI == lcm_params->type){//DBI
        DISP_DRV_INFO("DISP_Change_Update():DBI interface\n");        
        LCD_CHECK_RET(LCD_Change_WriteCycle(ctrl_if, speed));
    }
    else if(LCM_TYPE_DPI == lcm_params->type){//DPI
        DISP_DRV_INFO("DISP_Change_Update():DPI interface\n");
        DPI_CHECK_RET(DPI_Change_CLK(speed));
    }
    else if(LCM_TYPE_DSI == lcm_params->type){// DSI
        DISP_DRV_INFO("DISP_Change_Update():DSI interface\n");
...
DISP_STATUS DISP_ConfigAssertLayerMva()
{
    ...
    DISP_DRV_INFO("DAL Layer PA = DAL_layerPA = 0x%x, MVA = 0x%x\n", dal_layerPA, mva);
...
DISP_STATUS DISP_AllocUILayerMva(unsigned int pa, unsigned int *mva, unsigned int size)
{
    ...
    DISP_DRV_INFO("UI Layer PA = 0x%x, MVA = 0x%x\n", pa, *mva);
    if(LCM_TYPE_DPI == lcm_params->type){ //dpi buffer
        ...
        for(i=0;i<lcm_params->dpi.intermediat_buffer_num;i++){
            ...
            DISP_DRV_INFO("dpi pa = 0x%x,size = %d, mva = 0x%x\n", pa + size + i * dpi_size, dpi_size, dpi_mva);
    ...
    if((LCM_TYPE_DSI == lcm_params->type) && (lcm_params->dsi.mode != CMD_MODE)){ //dsi buffer
        ...
        for(i=0;i<lcm_params->dsi.intermediat_buffer_num;i++){
            ...
            DISP_DRV_INFO("dsi pa = 0x%x,size = %d, mva = 0x%x\n", pa + size + i * dsi_size, dsi_size, dsi_mva);
...
DISP_STATUS DISP_ChangeLCDWriteCycle()// this function is called when bootanimation start and stop in Surfaceflinger on 6573, because 6573 LCDC DBI write speed not stable when bootup
{
    ...
    if(LCM_TYPE_DBI == lcm_params->type){//DBI
        DISP_DRV_INFO("DISP_ChangeLCDWriteCycle():DBI interface\n");        
        LCD_CHECK_RET(LCD_Change_WriteCycle_ex(ctrl_if));
    }
    else{//DPI and DSI has not this issue, so not need change
        DISP_DRV_WRAN("DPI or DSI interface not need change write cycles\n");
    }
#endif
    return DISP_STATUS_OK;
}
...
{
    BOOL result = FALSE;
    DISP_DRV_INFO("DISP_EsdRecover enter");
    ...
    if(lcm_params->type==LCM_TYPE_DSI)
    {
     ...
     DISP_DRV_INFO("DISP_EsdRecover do LCM recover");
