...
static bool CheckSize(uint32 size, uint32 base, uint end)
{
    if((size) == 0 || size < base || size >= end){
        PRINTK_AUDDRV("[Auddrv][Warn]CheckSize size = %p",size);
        return true;
    }
    return false;
}
...
static int AudDrv_Read_Procmem(char *buf,char **start, off_t offset, int count , int *eof, void *data)
{
    ...
    len += sprintf(buf+len ,"AFE_MEMIF_MON0  = 0x%x\n",Afe_Get_Reg(AFE_MEMIF_MON0));
    len += sprintf(buf+len ,"AFE_MEMIF_MON1  = 0x%x\n",Afe_Get_Reg(AFE_MEMIF_MON1));
    len += sprintf(buf+len ,"AFE_MEMIF_MON2  = 0x%x\n",Afe_Get_Reg(AFE_MEMIF_MON2));
    len += sprintf(buf+len ,"AFE_MEMIF_MON4  = 0x%x\n",Afe_Get_Reg(AFE_MEMIF_MON4));
    ...
    len += sprintf(buf+len ,"AFE_UP8X_FIFO_CFG0  = 0x%x\n",Ana_Get_Reg(AFE_UP8X_FIFO_CFG0));
    len += sprintf(buf+len ,"AFE_UP8X_FIFO_LOG_MON0  = 0x%x\n",Ana_Get_Reg(AFE_UP8X_FIFO_LOG_MON0));
    len += sprintf(buf+len ,"AFE_UP8X_FIFO_LOG_MON1  = 0x%x\n",Ana_Get_Reg(AFE_UP8X_FIFO_LOG_MON1));
    len += sprintf(buf+len ,"AFE_PMIC_NEWIF_CFG0  = 0x%x\n",Ana_Get_Reg(AFE_PMIC_NEWIF_CFG0));
    len += sprintf(buf+len ,"AFE_PMIC_NEWIF_CFG1  = 0x%x\n",Ana_Get_Reg(AFE_PMIC_NEWIF_CFG1));
    len += sprintf(buf+len ,"AFE_PMIC_NEWIF_CFG2  = 0x%x\n",Ana_Get_Reg(AFE_PMIC_NEWIF_CFG2));
    len += sprintf(buf+len ,"AFE_PMIC_NEWIF_CFG3  = 0x%x\n",Ana_Get_Reg(AFE_PMIC_NEWIF_CFG3));

    len += sprintf(buf+len ,"AFE_TOP_PMIC_CON0  = 0x%x\n",Ana_Get_Reg(AFE_TOP_PMIC_CON0));
    len += sprintf(buf+len ,"ABB_MON_DEBUG0  = 0x%x\n",Ana_Get_Reg(ABB_MON_DEBUG0));
...
void Auddrv_Handle_Mem_context(AFE_MEM_CONTROL_T *Mem_Block)
{
    ...
    if(CheckSize (HW_Cur_ReadIdx, mBlock->pucPhysBufAddr, mBlock->pucPhysBufAddr + mBlock->u4BufferSize))
        return;
...
void Auddrv_DL_Interrupt_Handler(void)  // irq1 ISR handler
{
    ...
    {
        PRINTK_AUDDRV("[Auddrv][Warn] HW_Cur_ReadIdx ==0x%x, PhyAddr=0x%x, VirAddr-0x%x\n",HW_Cur_ReadIdx, Afe_Block->pucPhysBufAddr, Afe_Block->pucVirtBufAddr);
        PRINTK_AUDDRV("[Auddrv][Warn] AFE_DL1_BASE ==0x%x, AFE_DL1_END=0x%x\n",Afe_Get_Reg(AFE_DL1_BASE), Afe_Get_Reg(AFE_DL1_END));
    ...
    if((Afe_consumed_bytes & 0x3) != 0 ){
        PRINTK_AUDDRV("[Auddrv] MEMIF address is not aligned 4 bytes \n");
     }
     /*
...
static irqreturn_t AudDrv_IRQ_handler(int irq, void *dev_id)
{
    ...
    if(u4RegValue ==0 ){
        ...
        AudDrv_Clk_On();
        Afe_Set_Reg (AFE_IRQ_CLR, (1<<6)|(1<<1)|1 ,0xff);
        AudDrv_Clk_Off();
...
{
    printk(KERN_EMERG "+AudDrv_resume");
    AudDrv_Clk_On();
...
void Auddrv_Set_MemIF_Fp(struct file *fp,unsigned long arg)
{
    ...
    {
        PRINTK_AUDDRV("+ pAfe_MEM_ConTrol flip = %p fp = %p ,Error !!! \n",pAfe_MEM_ConTrol->flip,fp);
        return;
    }
...
void Auddrv_Release_MemIF_Fp(struct file *fp,unsigned long arg)
{
    ...
    {
        PRINTK_AUDDRV("+ pAfe_MEM_ConTrol Error !!! flip = %p fp = %p\n",pAfe_MEM_ConTrol->flip,fp);
        return;
    }
...
static long AudDrv_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case SET_AUDSYS_REG:{
            ...
            AudDrv_Clk_On();
            spin_lock(&auddrv_lock);
            PRINTK_AUDDRV("+AudDrv SET_AUDSYS_REG 0x%x 0x%x 0x%x, \n",Reg_Data.offset, Reg_Data.value, Reg_Data.mask);
        ...
        case AUDDRV_GET_AUXADC_CHANNEL_VALUE:
        {
            ...
            printk(KERN_EMERG "+AudDrv_GetAuxAdc arg %d, adc_return 0x%x", arg, adc_return);
            ret = adc_return;
            break;
        }
...
static ssize_t AudDrv_write(struct file *fp, const char __user *data, size_t count, loff_t *offset)
{
    ...
    /*
    PRINTK_AUDDRV("AudDrv_write Afe_Block->pucVirtBufAddr 0x%p, WriteIdx=%x, ReadIdx=%x, DataRemained=%x \n",
                Afe_Block->pucVirtBufAddr, Afe_Block->u4WriteIdx, Afe_Block->u4DMAReadIdx,Afe_Block->u4DataRemained);*/
   ...
   while(count)
   {
       ...
       if(copy_size != 0)
       {
           ...
           if(Afe_WriteIdx_tmp > Afe_Block->u4BufferSize){
               PRINTK_AUDDRV("[AudDrv][Warn]mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %x data_w_ptr = %p copy_size = %x\n",
