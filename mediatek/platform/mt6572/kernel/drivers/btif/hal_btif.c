...
int mtk_btif_dbg_lvl = BTIF_LOG_INFO;
...
static int btif_rx_irq_handler(P_MTK_BTIF_INFO_STR p_btif_info, unsigned char* p_buf, const unsigned int max_len);
...
*****************************************************************************/
static int _btif_receive_data(P_MTK_BTIF_INFO_STR p_btif, unsigned char* p_buf, const unsigned int max_len);
...
static int _btif_tx_fifo_init(P_MTK_BTIF_INFO_STR p_btif_info)
{
    ...
    {
        p_btif_info->p_tx_fifo = kzalloc(sizeof(struct kfifo), GFP_ATOMIC);            
        if (NULL == p_btif_info->p_tx_fifo)            
        {                
            i_ret = -ENOMEM;
        ...
        i_ret = kfifo_alloc(p_btif_info->p_tx_fifo, BTIF_HAL_TX_FIFO_SIZE, GFP_ATOMIC);
        if (0 != i_ret){
	...
	{
	    BTIF_WARN_FUNC("p_btif_info->p_tx_fifo is already init p_btif_info->p_tx_fifo(0x%x)\n", p_btif_info->p_tx_fifo);
		i_ret = 0;
	}
	ret:
	return i_ret;
...
int hal_btif_clk_ctrl(P_MTK_BTIF_INFO_STR p_btif, ENUM_CLOCK_CTRL flag)
{
    ...
    if (CLK_OUT_ENABLE == flag)
    {
        if(1 == atomic_inc_return(&s_clk_ref))
        {
            ...
            if (i_ret)
            {
            	BTIF_WARN_FUNC("enable_clock for MTK_BTIF_CG_BIT failed, ret:%d", i_ret);
            }
        }
    }
    else if (CLK_OUT_DISABLE == flag)
    {
        if(0 == atomic_dec_return(&s_clk_ref))
        {
            ...
            if (i_ret)
            {
                BTIF_WARN_FUNC("disable_clock for MTK_BTIF_CG_BIT failed, ret:%d", i_ret);
            }
        }
    }
    else
...
int hal_btif_irq_handler(P_MTK_BTIF_INFO_STR p_btif, unsigned char* p_buf, const unsigned int max_len)
{
    ...
    {
        rx_len += btif_rx_irq_handler(p_btif, (p_buf + rx_len), (max_len - rx_len));
...
int hal_btif_send_data(P_MTK_BTIF_INFO_STR p_btif, const unsigned char* p_buf, const unsigned int buf_len)
{
    ...
    if (0 < sent_len)
    {
		...
		unsigned long flag = 0;
		spin_lock_irqsave(&(p_btif->tx_fifo_spinlock), flag);
		enqueue_len = kfifo_in(p_btif->p_tx_fifo, (unsigned char *)p_buf, sent_len);
...
*  positive means data is available, 0 means no data available
*****************************************************************************/
int _btif_receive_data(P_MTK_BTIF_INFO_STR p_btif, unsigned char* p_buf, const unsigned int max_len)
{
    /*Chaozhong: To be implement*/
    int i_ret = -1;
    /*check parameter valid or not*/
