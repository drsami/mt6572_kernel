...
static int btif_rx_dma_ctrl(P_MTK_DMA_INFO_STR p_dma_info, ENUM_DMA_CTRL ctrl_id);
static int btif_tx_dma_ctrl(P_MTK_DMA_INFO_STR p_dma_info, ENUM_DMA_CTRL ctrl_id);
...
static int hal_rx_dma_dump_reg(P_MTK_DMA_INFO_STR p_dma_info, ENUM_BTIF_REG_ID flag);
static int hal_tx_dma_dump_reg(P_MTK_DMA_INFO_STR p_dma_info, ENUM_BTIF_REG_ID flag);
...
*****************************************************************************/
static int hal_dma_receive_data(P_MTK_DMA_INFO_STR p_dma_info, unsigned char* p_buf, const unsigned int max_len);
...
int hal_btif_dma_clk_ctrl(P_MTK_DMA_INFO_STR p_dma_info, ENUM_CLOCK_CTRL flag)
{
    ...
    if (CLK_OUT_ENABLE == flag)
    {
        if(1 == atomic_inc_return(&s_clk_ref))
        {
			...
			if (i_ret)
            {
                BTIF_WARN_FUNC("enable_clock for MTK_BTIF_APDMA_CLK_CG failed, ret:%d", i_ret);
            }
        }
    }
	else if (CLK_OUT_DISABLE == flag)
	{
	    if(0 == atomic_dec_return(&s_clk_ref))
        {
			...
			if (i_ret)
            {
                BTIF_WARN_FUNC("disable_clock for MTK_BTIF_APDMA_CLK_CG failed, ret:%d", i_ret);
            }
        }
	}
	else
    ...
    {
        i_ret = 0;
        BTIF_DBG_FUNC("dma clock already %s\n", CLK_OUT_ENABLE == status ? "enabled" : "disabled");
    }
    else
    {
        if (CLK_OUT_ENABLE == flag)
    ...
    {
        BTIF_ERR_FUNC("%s dma clock failed, ret(%d)\n", CLK_OUT_ENABLE == flag ? "enable" : "disable", i_ret);
    }
#else
...
int hal_btif_dma_hw_init(P_MTK_DMA_INFO_STR p_dma_info)
{
    ...
    P_MTK_BTIF_DMA_VFIFO p_mtk_dma_vfifo = container_of(p_vfifo, MTK_BTIF_DMA_VFIFO, vfifo);
    ...
    if (DMA_DIR_RX == p_dma_info->dir)
    {
        ...
        btif_reg_sync_writel(p_mtk_dma_vfifo->wpt, RX_DMA_VFF_WPT(base));
    ...
    else
    {
        ...
        /*write wpt to VFF_WPT*/
        btif_reg_sync_writel(p_mtk_dma_vfifo->wpt, TX_DMA_VFF_WPT(base));
        /*write vff_thre to VFF_THRESHOLD*/ 
...
static int is_tx_dma_irq_finish_done (P_MTK_DMA_INFO_STR p_dma_info)
{
	...
	tx_irq_done = (0 == (BTIF_READ32(TX_DMA_INT_FLAG(base))  & TX_DMA_INT_FLAG_MASK)) ? 1 : 0;
...
#define MAX_CONTINIOUS_TIMES 512
    ...
    unsigned int base = p_dma_info->base;
    static int flush_irq_counter = 0;
	...
	{
		spin_unlock_irqrestore(&(g_clk_cg_spinlock), flag);
		BTIF_ERR_FUNC("%s: clock is off before irq status clear done!!!\n", __FILE__);
        return i_ret;
	}
...
int hal_dma_send_data(P_MTK_DMA_INFO_STR p_dma_info, const unsigned char* p_buf, const unsigned int buf_len)
{
    ...
    P_MTK_BTIF_DMA_VFIFO p_mtk_vfifo = container_of(p_vfifo, MTK_BTIF_DMA_VFIFO, vfifo);
    ...
    if (buf_len > ava_len)
    {
        BTIF_ERR_FUNC("length to send:(%d) < length available(%d), abnormal!!!---!!!\n", buf_len, ava_len);
    ...
    if (len_to_send + wpt >= vff_size)
    {
        ...
        p_data += tail_len;
        memcpy (p_mtk_vfifo->vfifo.p_vir_addr, p_data, len_to_send - tail_len);
    ...
    {
        memcpy ((p_mtk_vfifo->vfifo.p_vir_addr + wpt), p_data, len_to_send);
        /*make sure all data write to memory area tx vfifo locates*/
        dsb();
    ...
    {   
        ...
        if ((8 > BTIF_READ32(TX_DMA_VFF_VALID_SIZE(base))) && (0 < BTIF_READ32(TX_DMA_VFF_VALID_SIZE(base))) )
        {
    ...
    {
        /*TODO: print error log*/
        BTIF_ERR_FUNC("Tx DMA flush operation is in process, this case should never happen, please check if tx operation is allowed before call this API\n");
...
bool hal_dma_is_tx_complete(P_MTK_DMA_INFO_STR p_dma_info)
{
    ...
    */
    ...
    {
        BTIF_DBG_FUNC("DMA tx is in process. vfifo valid size(%d), dma internal size (%d), tx_done(%d)\n", \
            valid_size, \
            inter_size, \
            tx_done);
        b_ret = false;
...
*  positive means data is available, 0 means no data available
*****************************************************************************/
int hal_dma_receive_data(P_MTK_DMA_INFO_STR p_dma_info, unsigned char* p_buf, const unsigned int max_len)
{
    unsigned int i_ret = -1;
...
int hal_rx_dma_irq_handler(P_MTK_DMA_INFO_STR p_dma_info, unsigned char* p_buf, const unsigned int max_len)
{
    ...
    {
        BTIF_DBG_FUNC("rx interrupt, no data available in Rx DMA, wpt(0x%08x), rpt(0x%08x)\n", rpt, wpt);
    }
    ...
    while ((0 < valid_len) || (rpt != wpt))
    {
        ...
        if (NULL != rx_cb)
        {
            ...
            else
            {
                ...
                p_vff_buf = vff_base;
                (*rx_cb)(p_dma_info, p_vff_buf, real_len - tail_len);
            }
            i_ret += real_len;
        }
        else
...
static int hal_tx_dma_dump_reg(P_MTK_DMA_INFO_STR p_dma_info, ENUM_BTIF_REG_ID flag)
{
    ...
    BTIF_INFO_FUNC("tx dma %s\n", (enable & DMA_EN_BIT) && (!(stop && DMA_STOP_BIT)) ? "enabled" : "stoped");
